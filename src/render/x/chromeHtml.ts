...
export function buildChromeHtml(assets: XPostAssets, layout: XPostLayout): string {
  ...
  const headerTop = 33;
  const avatarSize = 72;
  const nameLine = 40; // 32px font / 40px line-height
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .avatar.placeholder {
    ...
    font-weight: 700; font-size: 32px; color: #71767a;
  }
  .main { flex: 1; min-width: 0; }
  .name-row { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
  .name { font-weight: 700; font-size: 32px; line-height: 40px; color: #e6e9ea; }
  .handle { font-size: 32px; line-height: 40px; color: #71767a; font-weight: 400; }
  .badge { flex-shrink: 0; }
  .text {
    margin-top: 8px;
    font-size: 32px;
  ...
  .quote .name, .quote .handle { font-size: 28px; line-height: 36px; }
  .qtext { margin-top: 8px; font-size: 30px; line-height: 38px; color: #e6e9ea; }
  .qimgs {
    margin-top: 12px;
    display: grid;
